...
for(char of text){
  ...
  if ((characterScript(codepoint).direction)=="ltr"){
  numltr+=1
  }
  else if ((characterScript(codepoint).direction)=="rtl"){
    numrtl+=1
    }
    else if ((characterScript(codepoint).direction)=="ttb"){
      numttb+=1
      }

   
}
sum = numltr+numrtl+numttb;
console.log("Persantage of ltr is "+ Math.round(100*numltr/sum));
console.log("Persantage of rtl is "+ Math.round(100*numrtl/sum));
console.log("Persantage of ttb is "+ Math.round(100*numttb/sum));

}
drd("UYTuytUgmcmsnmkjcnwjncneывтвдлдат");
drd("Hey, الخير");
